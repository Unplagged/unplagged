...
class CaseController extends BaseController{
  ...
  /**
   * Provides the form to create a case and creates it when the data
   * was valid.
   * 
   * @todo there seems to be a zf2 bug here with the population of data after
   * filtering
   */
  public function createAction(){
    $formConfig = include_once __DIR__ . '/../../config/forms/case-create.php';
    $factory = new Factory();
    $createForm = $factory->createForm($formConfig);
    $request = $this->getRequest();
    $data = $request->getPost();
    $createForm->setData($data);

    if($request->isPost() && $createForm->isValid()){
      $validatedData = $createForm->getData();

      $user = $this->zfcUserAuthentication()->getIdentity();
      $case = EntityFactory::createCase($this->em, $validatedData['case-name'], $validatedData['case-alias'],
                      $validatedData['case-description'], $user);
      $this->activityStream()->publishActivity('The case {result.publishableName} was created by {actor.name}', $user,
              'The case {result.publishableName} was created by you', '', $case);
      $user->setCurrentCase($case);

      $this->flashMessenger()
              ->setNamespace('info')
              ->addMessage(sprintf($this->getTranslator()->translate('The Case %s was created successfully.'),
                              $case->getPublishableName()));
      $this->redirect()->toRoute('view-case', array('id'=>$case->getId()));
    }
    $createForm->setAttribute('action', $this->url()->fromRoute('create-case'));
    $createForm->setAttribute('method', 'post');
    $createForm->setAttribute('class', $formConfig['display_options']['class']);

    $actions = array(
        $createForm->get('submit-btn'),
        $createForm->get('reset-btn'),
    );
    $createForm->prepare();
    return array(
        'form'=>$createForm,
        'formType'=>'horizontal',
        'displayOptions'=>$formConfig['display_options'],
        'actions'=>$actions,
    );
  }

  /**
   * Displays a list of documents of the type specified in the url parameter
   * and for the given case id.
   * 
   * @return array|Response
   */
  public function documentsAction(){
    $id = $this->params('case_id');
    $type = $this->params('document_type');
    if($id && $type){
      ...
      switch($type){
        case 'sources':
          $documents = $case->getSources();
          $title = sprintf($this->getTranslator()->translate('Source Documents of %s'), $case->getPublishableName());
          break;
        case 'targets':
          $documents = $case->getTargetDocuments();
          $title = sprintf($this->getTranslator()->translate('Investigation Documents of %s'),
                  $case->getPublishableName());
          break;
        default:
