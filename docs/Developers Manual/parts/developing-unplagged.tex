...
To resolve the issues, open the files listed in the error message, in this case \textit{readme.txt} and decide how the correct 
...
in listing \ref{list:redmineGithubHook} so that it also calls the below bash script(listing \ref{deploymentScript})
...
The above described workflow is already nice, but it still has a lot of room for improvement.

\begin{itemize}
\item Email notification on failed unit test after the deployment on the server
\item Installer
\item Separation into a staging environment with the newest commits and an actual preview environment, that can be deployed
by a simple script and the commit id
\end{itemize}
