...
\subsection{Netbeans}
As a software developer who wants to develop object-oriented and use a free software, there is acutally not so much software to choose from available. Either Eclipse or Netbeans, we decided to use Netbeans since it has more built-in plugins for PHP and it is faster.

This section will describe how to configure Netbeans in order to start working on the Unplagged project. It is necessarry that PHPUnit is already installed on the machine, otherwise not all steps of the Netbeans setup can be done.

\subsubsection{Setting up PHPUnit}

\begin{description}
\item [Mac OS:] \hfill \\
\url{http://kubyshkin.ru/programming/phpunit-on-mac-os-x-snow-leopard-10-6/} \\
\url{http://www.newmediacampaigns.com/page/install-pear-phpunit-xdebug-on-macosx-snow-leopard} \\
\url{http://www.phpunit.de/manual/3.6/en/code-coverage-analysis.html}

\item [Windows:] \hfill \\
For windows there a are some more steps to. First of all it is assumed that XAMPP is already installed.
\end{description}

\begin{lstlisting}[caption=Setting up PHPUnit on Windows]
# download http://pear.php.net/go-pear.phar and save it to
C:\xampp\php
...
Download the latest Netbeans release for PHP from the Netbeans webpage:\\
\url{http://netbeans.org/downloads/}

\begin{lstlisting}[caption=Setting up Netbeans project]
Install it and go to File > New Project
Choose PHP > PHP Appliction with Existing Sources
Set "Sources Folder" to the directory, where the git repository is located.
"Project Name" can e.g. be "unplagged.com"
"PHP Version" should be "5.3"
"Default Encoding" "UTF-8"
IMPORTANT: Check the Checkbox "Put NetBeans metadata into a seperate directory" and choose another directory than the git repository.
\end{lstlisting}
